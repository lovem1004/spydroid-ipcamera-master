...
public class SessionBuilder {
	...
	private int mCamera_back = CameraInfo.CAMERA_FACING_BACK;
	private int mTimeToLive = 64;
	private int mOrientation = 0;
	private boolean mFlash = false;
	private SurfaceView mSurfaceView = null;
	private SurfaceView mSurfaceView_back = null;
	...
	private static volatile SessionBuilder sInstance_back = null;
	...
	public final static SessionBuilder getInstance_back() {
		if (sInstance_back == null) {
			synchronized (SessionBuilder.class) {
				if (sInstance_back == null) {
					SessionBuilder.sInstance_back = new SessionBuilder();
				}
			}
		}
		return sInstance_back;
	}
	...
	public Session build_back() {
		...
		switch (mVideoEncoder) {
			case VIDEO_H263:
				session.addVideoTrack(new H263Stream(mCamera_back));
				break;
			case VIDEO_H264:
				H264Stream stream = new H264Stream(mCamera_back);
				if (mContext!=null)
					stream.setPreferences(PreferenceManager.getDefaultSharedPreferences(mContext));
				session.addVideoTrack(stream);
				break;
	...
	public SessionBuilder clone_back() {
		return new SessionBuilder()
				.setDestination(mDestination)
				.setOrigin(mOrigin)
				.setSurfaceView(mSurfaceView)
