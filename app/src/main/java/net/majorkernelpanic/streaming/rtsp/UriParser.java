...
public class UriParser {
	...
	public static Session parse(String uri) throws IllegalStateException, IOException {
		Log.e(TAG, "--------------------------------guoyuefeng111111 Session parse");
		SessionBuilder builder = SessionBuilder.getInstance().clone();
		byte audioApi = 0, videoApi = 0;

		List<NameValuePair> params = URLEncodedUtils.parse(URI.create(uri),"UTF-8");
		Log.e(TAG, "--------------------------------guoyuefeng111111 params.size() = " + params.size());
		if (params.size()>0) {
			...
			for (Iterator<NameValuePair> it = params.iterator();it.hasNext();) {
				...
				Log.e(TAG, "--------------------------------guoyuefeng111111 param.getName() = " + param.getName());
				Log.e(TAG, "--------------------------------guoyuefeng111111 param.getValue() = " + param.getValue());
				// FLASH ON/OFF
	...
	public static Session parse_back(String uri) throws IllegalStateException, IOException {
		Log.e(TAG, "--------------------------------guoyuefeng111111 Session parse");
		SessionBuilder builder = SessionBuilder.getInstance_back().clone_back();
		byte audioApi = 0, videoApi = 0;
		...
		if (builder.getVideoEncoder()==VIDEO_NONE && builder.getAudioEncoder()==AUDIO_NONE) {
			SessionBuilder b = SessionBuilder.getInstance_back();
			builder.setVideoEncoder(b.getVideoEncoder());
			builder.setAudioEncoder(b.getAudioEncoder());
		}

		Session session = builder.build_back();
