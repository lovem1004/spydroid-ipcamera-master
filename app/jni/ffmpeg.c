...
int main(int argc, char **argv)
{
    ...
    int i, ret;
    int64_t ti;
av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c 111\n");
    init_dynload();
av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c 222\n");
    register_exit(ffmpeg_cleanup);
av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c 333\n");
    setvbuf(stderr,NULL,_IONBF,0); /* win32 runtime needs this */

    av_log_set_flags(AV_LOG_SKIP_REPEATED);
    av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c 444\n");
    parse_loglevel(argc, argv, options);
av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c 555\n");
...
av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c 666\n");
    avcodec_register_all();
    av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c 777\n");
#if CONFIG_AVDEVICE
    ...
    av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c 888\n");

    show_banner(argc, argv, options);
    av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c 999\n");

    /* parse options and open all input/output files */
    ret = ffmpeg_parse_options(argc, argv);
    av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c aaa\n");
    if (ret < 0)
        exit_program(1);

    if (nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
        av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c Use -h to get full help or, even better, run 'man %s'\n", program_name);
        exit_program(1);
    }

av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c bbb\n");
    /* file converter / grab */
    if (nb_output_files <= 0) {
        av_log(NULL, AV_LOG_FATAL, "guoyuefeng ffmpeg.c At least one output file must be specified\n");
        exit_program(1);
    }
av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c ccc\n");
//     if (nb_input_files == 0) {
...
av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c ddd\n");
    current_time = ti = getutime();
    av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c ddd111\n");
    if (transcode() < 0) {
        av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c ddd222\n");
        exit_program(1);
    }
