...
int muxer_main(char *inputH264FileName,char *inputAacFileName,char *outMP4FileName,char* angle)
{
    ...
    if ((ret = avformat_open_input(&ifmt_ctx_v, in_filename_v, NULL, &options)) < 0) {
            ...
            if(av_read_frame(ifmt_ctx, &pkt) >= 0){
                do{
                    ...
                    if(pkt.stream_index==videoindex_v){
                        ...
                        #if USE_H264BSF
                            new_pkt = pkt;
                            int  ret = av_bitstream_filter_filter(h264bsfc, in_stream->codec, NULL, &new_pkt.data, &new_pkt.size, pkt.data, pkt.size, 0);
            ...
            if(av_read_frame(ifmt_ctx, &pkt) >= 0){
                do{
                    ...
                    if(pkt.stream_index==audioindex_a){
                        ...
                        #if USE_AACBSF
                            new_pkt = pkt;
                            int ret = av_bitstream_filter_filter(aacbsfc, out_stream->codec, NULL, &new_pkt.data, &new_pkt.size, pkt.data, pkt.size, 0);
                            pkt.data = new_pkt.data;
                            pkt.size = new_pkt.size;
                            if (ret < 0) {
                                 av_packet_unref(&pkt);
...
int main(int argc, char **argv)
{
    ...
    int i, ret;
    int64_t ti;
//av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c 1111111\n");
    init_dynload();
//av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c 222\n");
    register_exit(ffmpeg_cleanup);
//av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c 333\n");
    setvbuf(stderr,NULL,_IONBF,0); /* win32 runtime needs this */

    av_log_set_flags(AV_LOG_SKIP_REPEATED);
    //av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c 444\n");
    parse_loglevel(argc, argv, options);
//av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c 555\n");
...
//av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c 666\n");
    avcodec_register_all();
    //av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c 777\n");
#if CONFIG_AVDEVICE
    ...
    //av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c 888\n");

    show_banner(argc, argv, options);
    //av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c 999\n");

    /* parse options and open all input/output files */
    ret = ffmpeg_parse_options(argc, argv);
    //av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c aaa\n");
    if (ret < 0)
        exit_program(1);

    if (nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
        av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c Use -h to get full help or, even better, run 'man %s'\n", program_name);
        exit_program(1);
    }

//av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c bbb\n");
    /* file converter / grab */
    if (nb_output_files <= 0) {
        av_log(NULL, AV_LOG_FATAL, "david ffmpeg.c At least one output file must be specified\n");
        exit_program(1);
    }
//av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c ccc\n");
//     if (nb_input_files == 0) {
...
//     }
    ...
    //av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c ddd\n");
    current_time = ti = getutime();
    //av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c ddd111\n");
    if (transcode() < 0) {
        //av_log(NULL, AV_LOG_WARNING, "david ffmpeg.c ddd222\n");
        exit_program(1);
    }
    //av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c ddd333\n");
    ti = getutime() - ti;
    //av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c ddd444\n");
    if (do_benchmark) {
        av_log(NULL, AV_LOG_INFO, "guoyuefeng ffmpeg.c bench: utime=%0.3fs\n", ti / 1000000.0);
    }
    av_log(NULL, AV_LOG_DEBUG, "%"PRIu64" guoyuefeng ffmpeg.c frames successfully decoded, %"PRIu64" decoding errors\n",
           decode_error_stat[0], decode_error_stat[1]);
    if ((decode_error_stat[0] + decode_error_stat[1]) * max_error_rate < decode_error_stat[1]) {
        //av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c ddd555\n");
        exit_program(69);
    }
//av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c ddd666\n");
    //remove by guoyuefeng
    //exit_program(received_nb_signals ? 255 : main_return_code);
    //av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c eee\n");
    ffmpeg_cleanup(0);
    //av_log(NULL, AV_LOG_WARNING, "guoyuefeng ffmpeg.c fff\n");
